...
#
# Name          : Lag-H
# Collaborators : watdoyumean
# Time spent    : A lot idk why, i'm excruciatingly slow
...
def is_valid_word(word, hand, word_list):
    ...
    word_in_list = False
    if word_dict.get('*', 0) > 0:
        for x in VOWELS:
            temp = word[:word.find('*')].lower() + x + word[word.find('*')+1:].lower()
    ...
    if word_in_hand and word_in_list:
        return True
    else:
        return False
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(input_word, hand, word_list):
                ...
                print("Your word earned you", get_word_score(input_word, calculate_handlen(hand)), "points.", "Total:", score)
            # Otherwise (the word is not valid):
            else:
                # Reject invalid word (print a message)
                print("You entered an invalid word. Please enter another word.")
...
def substitute_hand(hand, letter):
    ...
    if new_hand.get(letter, 0):
        x = random.choice(random.choice([VOWELS, CONSONANTS]))
        while new_hand.get(letter, 0) != 0:
            if new_hand.get(x, -1) == -1:
